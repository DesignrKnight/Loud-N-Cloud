...
import updateFilter from "../services/update-vaccine";
...
class PopRenderMap extends Component {
  ...
  async componentWillReceiveProps(newProps) {
    ...
    if (files) {
      ...
      console.log("Abel",curstate);
      let apiparam = "{";
      for (let key of Object.keys(curstate)) {
        if (key == "vaccine" && curstate.vaccine == "high")
          apiparam = apiparam + `'Normalized_Vaccination_Priority':'descend',`;
        else if (key == "vaccine" && curstate.vaccine == "low")
      ...
      if (apiparam == "{}") {
        ...
        let api =
          "https://project-manna-backend.herokuapp.com/getRowsSortByCol/" +
          apiparam;
        console.log(api);
        ...
        let vaccineNeeded=updateFilter(districts, data, curstate.state);
        let tempstate=this.state;
        tempstate.vaccineCount=vaccineNeeded
    ...
    return (
      <div key={this.props} id="demobox">
        <h3>Vaccines Needed</h3> 
          ...
          <h6><b>State:</b>{this.state.currentState.state}</h6>
          <h6><b>Priority:</b>{this.state.currentState.vaccine}</h6>
        <div id="map">
          <div id="select"></div>
        </div>
      </div>
